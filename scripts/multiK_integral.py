...
try:
  ...
  def is_processed(s):
    ...
    try:
      ...
      print "Failed to read %s"%structs1[s]
      print "Error:"
      raise
    try:
      B = read_infile(structs2[s])
    except ValueError:
      print "Failed to read %s"%structs2[s]
      print "Error:"
      raise
...
for s in structs:
  ...
  try:
    ...
    KAB_k  = KAB[np.nanargmax(np.abs(KAB_z),axis=0)]
    KAD_k  = KAD[np.nanargmax(np.abs(KAD_z),axis=0)]
    KBD_k  = KBD[np.nanargmax(np.abs(KBD_z),axis=0)]

    # Calculate the integral of the empirical medians
    DABp_integral = simps(np.median(DABp,axis=0),x=T)
    KABp_integral = simps(np.median(KABp,axis=0),x=T)
    KADp_integral = simps(np.median(KADp,axis=0),x=T)
    KBDp_integral = simps(np.median(KBDp,axis=0),x=T)

    # Calculate the area between each permutation and the median
    DABsp = np.apply_along_axis(simps,1,DABp,x=T) - DABp_integral
    KABsp = np.apply_along_axis(simps,1,KABp,x=T) - KABp_integral
    KADsp = np.apply_along_axis(simps,1,KADp,x=T) - KADp_integral
    KBDsp = np.apply_along_axis(simps,1,KBDp,x=T) - KBDp_integral
    P,DABs_p,DABs_z,DABs_zp = pstat(DABsp)
    P,KABs_p,KABs_z,KABs_zp = pstat(KABsp)
    P,KADs_p,KADs_z,KADs_zp = pstat(KADsp)
    P,KBDs_p,KBDs_z,KBDs_zp = pstat(KBDsp)
    ...
    # Summary of K*
    res = [sid,chain,NA,NB,KAB_k,KABs_p,KABs_z,KABs_zp,KAB_t]
    print "\nSummary of K*:"
    ...
    with open("%s/%s_KAB_summary.txt"%(args.outdir,args.label),'ab') as fout:
      ...
      csv.writer(fout,delimiter='\t').writerow(res)
    # Summary of Ka - K*
    res = [sid,chain,NA,NB,KAD_k,KADs_p,KADs_z,KADs_zp,KAD_t]
    print "\nSummary of Ka - K*:"
    ...
    with open("%s/%s_KAD_summary.txt"%(args.outdir,args.label),'ab') as fout:
      ...
      csv.writer(fout,delimiter='\t').writerow(res)
    #Summary of Kb - K*      
    res = [sid,chain,NA,NB,KBD_k,KBDs_p,KBDs_z,KBDs_zp,KBD_t]
    print "\nSummary of Kb - K*:"
