...
def sanitize_data(pdb_id,seqadv_protected):
	#print("\nPerforming QC analysis for %s."%pdb_id)

	# Track the number of issues this PDB ID has
	num_issues = 0
	...
	for transcript,chain,homologue in pdb_t:
		...
		# Track the number of issues with this transcript
		mismatch_issues = []
		range_issues    = []
		num_issues      = 0
		...
		for i,seqres in enumerate(sorted_keys):
			...
			# Or the amino acid is different at that seqres, discard
				...
				# print("\t\t#-------------#")
				mismatch_issues.append(str(seqres))
				range_issues.append('-')
				sys.stderr.write("%2.2f\n"%(i/tot_len))
				num_issues      += 1
			else:
	...
	# if num_issues > 0:
	# 	print("PDB %s had %d transcripts with %d total conflicts."%(pdb_id,len(pdb_t),num_issues))
	# 	print("grep-tag\n")
