...
class PDBMap():
    ...
    def load_vcf_data(self,dlabel,vcf_filename):
        ...
        nrows_inserted = pdbmap_vep.vcf_file_to_sql_table(sql_table_name,vcf_filename,args.buffer_size if args.buffer_size else 1)
...
class Command_runner:
    ...
    def load_vcf_cmd(pdbmap,args,remaining_args):
        ...
        LOGGER.info("Processing vcf data in ENSEMBL VEP.  Adding completed VEP outputs to GenomicConsequence and GenomicData tables")
        nrows = 0
        for vcf_filename,dlabel  in dfiles:
...
|   |__/|__)|  |(_||_) 
    ...
    # first_cmdline_parser = argparse.ArgumentParser(prog='PROG')
    first_cmdline_parser = argparse.ArgumentParser(prog='PROG',formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    first_cmdline_parser.add_argument("-u", "--userconfig", default = getpass.getuser() + ".config",
        help="User specific config options to selectively override global config", metavar="FILE")
    ...
    subparser_load_vcf.add_argument("--buffer_size", type=int, default=100,
    ...
    subparser_confighelp = subparsers.add_parser('confighelp',help='List all the optional config file override parameters')
...
drwx------ 3 root   root             17 Oct 17 16:00 systemd-private-1461403187af4efe9331fee92595b522-chronyd.servicems if name not in config_dict]
    ...
    parser = argparse.ArgumentParser(parents=[first_cmdline_parser],description=__doc__,formatter_class=argparse.RawDescriptionHelpFormatter)
