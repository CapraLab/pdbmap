...
MIRRORDIR=/scratch/sivleyrm/pdb/all/pdb                 # your top level rsync directory
LOGFILE=/scratch/sivleyrm/logs               # file for storing logs
...
############################################################################
# Rsync only the PDB format coordinates  /pub/pdb/data/structures/all/pdb (Aproximately 9 GB)
############################################################################
${RSYNC} -rlpt -v -z --delete --port=$PORT ${SERVER}/data/structures/all/pdb/ $MIRRORDIR > $LOGFILE 2>/dev/null
