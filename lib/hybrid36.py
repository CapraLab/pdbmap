...
# Tweaked to Python3 better compatability and PEP-8 formatting
# By Chris Moth, 2020 November.
...
digits_upper_values = {char_value_pair[0]: char_value_pair[1] for char_value_pair in zip(digits_upper, range(36))}
digits_lower_values = {char_value_pair[0]: char_value_pair[1] for char_value_pair in zip(digits_lower, range(36))}
...
def decode_pure(digits_values: Dict[str, int], s: str) -> int:
    """decodes the string s using the digit, value associations for each character
    """
    result = 0
    n = len(digits_values)  # Typically 36 or 10
    for c in s:
        result *= n
...
def hy36encode(width: int, value: int) -> str:
    """encodes value as base-10/upper-case base-36/lower-case base-36 hybrid
    """
    # This code looks terribly inefficient - but I am not using encode
    # So I'm not worrying about the endless recomputation of same exponentials
...
if __name__ == "__main__":
    def exercise(hy36enc=hy36encode, hy36dec=hy36decode):
        for digits, digits_values in [(digits_upper, digits_upper_values),
            ...
            for value in range(1000):
                s = encode_pure(digits=digits, value=value)
                d = decode_pure(digits_values=digits_values, s=s)
                assert d == value

        #
        def recycle4(test_value_int: int, expected_encoded_str: str):
            """
            :param test_value_int:   int to encode and test
            :param expected_encoded_str: string which must == encoded test_value_int
            :return: None
            """
            _s = hy36enc(width=4, value=test_value_int)
            assert _s == expected_encoded_str
            _d = hy36dec(width=4, s=_s)
            assert _d == test_value_int
        ...
        #
        def recycle5(test_value_int: int, expected_encoded_str: str):
            """
            :param test_value_int:   int to encode and test
            :param expected_encoded_str: string which must == encoded test_value_int
            :return: None
            """
            _s = hy36enc(width=5, value=test_value_int)
            assert _s == expected_encoded_str
            _d = hy36dec(width=5, s=_s)
            assert _d == test_value_int
    ...
    run()
    print(__file__ + "Tests Completed Successfully")
