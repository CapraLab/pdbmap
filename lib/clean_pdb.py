...
if argv[2].strip() != "ignorechain" and argv[2].strip() != "nochain" and argv[2].strip() != "rechain":
...
outfile = pdbname[0:-4].lower() + "_" + chainid + ".pdb"
...
rechain_dict = {}
def generate_chain():
    generate_chain.cid += 1
    return chr(generate_chain.cid)
generate_chain.cid = 64
model = 0
...
# resnum = line_edit[23:26]
            ...
            if rechain:
                cid = line[21]
                #print "(%s %s %s) ->"%(model,cid,line[22:26]),
                if (model,cid) not in rechain_dict:
                    rechain_dict[(model,cid)] = generate_chain()
                cid = rechain_dict[(model,cid)]
                #print "%s %s"%(cid,line[22:26])
                line_edit = line_edit[0:21]+cid+line_edit[22:]
...
pdbname = pdbname.split('.')[0].split('/')[-1].upper()

if chainid == ' ':
    chainid = '_'
if nres > 0:
    if (nopdbout == 0):
        #outfile = ( basename(outfile) ).lower()
        outfile = outfile.lower().replace('.pdb1.gz', '.pdb')
   ...
   # outid.close()
    ...
    #fastaid = stdout
    #if argv[2] != "ignorechain" and argv[2] != "nochain" and argv[2] != "rechain":
