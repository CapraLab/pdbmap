...
class PDBMapSwiss(Structure):
    ...
    _JSON_fields = ["uniprot_ac", "template", "from", "to", "provider", "url", "qmean", "qmean_norm", "coordinate_id",
                    "iso_id"]
    swiss_dir = None
    method_resolution_re = re.compile('(^.*) ([0-9]\\.[0-9]*) A$')
  ...
  "run_name","mod_seq_id","mod_model_id","modelid","start","end","sequence_identity","evalue","ga341","mpqs","zdope","template_pdb", "template_chain","template_start","template_end","hit_history", "tsvmod_method","tsvmod_no35","tsvmod_rmsd","filename","unp"]
    ...
    def load_REMARK3_metrics(cls, modelid):
        ...
        with open(fname, 'rt') as f:
            for line in f:
                if line.startswith(
                        'ATOM  '):  # Then we're done parsing the pdb file.  ATOM comes after all REMARK lines of interest
                    break;
                if line.startswith('REMARK   3') and len(line) > 20:  # Then it's likely a swiss-specific model REMARK
                    ...
                    if len(splits) == 3:
                        ...
                        if (key != 'template'):
                            metrics[key] = value
                            if key == 'mthd':
                                method_resolution_resolution_match = PDBMapSwiss.method_resolution_re.match(metrics[key])
                                if method_resolution_resolution_match:
                                    metrics['mthd'] = method_resolution_resolution_match.group(1)
                                    metrics['resolution'] = float(method_resolution_resolution_match.group(2))
        return metrics
